...
- dispatches every retained row through its recorded immutable schema validator, rejects additional schema properties, requires every line to be its exact canonical JSON serialization, deep-validates derived summaries, repository binding, slot/time ordering, stable PR creation times, monotonic PR update times, and digest links, and rejects duplicate runs/source audits/slots before writing;
