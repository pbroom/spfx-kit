...
| Host React types                | `@types/react@17.0.45`, `@types/react-dom@17.0.17`                                  |
| Isolated profile compiler types | `@types/react@17.0.93`, `@types/react-dom@17.0.17`, `@types/scheduler@0.16.8`       |
...
[spfx-kit#82](https://github.com/pbroom/spfx-kit/issues/82) keeps the repository
and SPFx host contract on `@types/react@17.0.45` and
`@types/react-dom@17.0.17`. The private tooling-only profile compiler isolates
`@types/react@17.0.93`, `@types/react-dom@17.0.17`, and
`@types/scheduler@0.16.8` because Base UI 1.6.0 declarations use `React.JSX`,
which the host React-types pin does not provide.

The isolated compiler pin is not a host type upgrade, production dependency,
runtime change, or proof that normalized source compiles through the real SPFx
host path. Phase 1 exit still requires representative host/SPFx compilation
against the preserved `@types/react@17.0.45` contract. The profile ID remains
