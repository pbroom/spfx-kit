...
evidence required by its validation tier. Terminal phase and release evidence
is not repeated on every implementation PR.
...
## Critical-Path Delivery And Validation Cadence

Delivery and hardening run as separate lanes. A dependent phase may start when
the predecessor's consumed interface is frozen, its focused gates pass, and the
work remains stacked and rollback-isolated. This unblocks implementation; it
does not mark the predecessor complete. Phase acceptance remains sequential and
still requires every terminal event defined by this roadmap.

| Tier                   | When it runs                                                | Blocking evidence                                                                                                                                                                   |
| ---------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Focused implementation | Before each implementation PR is submitted                  | Touched-package typecheck/build, directly related tests, deterministic profile/digest checks when affected, touched keyboard/focus/accessibility behavior, and diff/format hygiene. |
| Phase integration      | Once at the frozen phase stack tip                          | Full repository gates, complete phase browser/Axe coverage, React singleton and dependency closure, applicable SPFx canary/package artifacts, and exact-head hosted CI.             |
| Release operations     | Only at the applicable release gate with explicit authority | Full export matrix, remote bytes and headers, App Catalog/test-site operations, SharePoint runtime trace, retention, fallback, and rollback proof.                                  |

Do not run the full repository, browser, artifact, or release matrix after
every repair commit. Batch local repairs, regenerate bound profile artifacts
once per stable candidate, and prefer one consolidated push so hosted CI and
automatic review run against meaningful review surfaces.

Each logical PR receives one clean-context adversarial review after focused
validation. A repair re-review first verifies the reported findings; it does
not restart unbounded discovery. A finding blocks the current delivery only
when it demonstrates a defect in the committed pinned inputs, generated
artifacts, supported public surface, touched runtime path, or a non-negotiable
rule. A future-input shape, unsupported optional syntax, maintainability
refinement, or defense-in-depth improvement that cannot affect the current
candidate is recorded as hardening with an owner and due gate. It becomes
blocking before that input or capability is admitted.

Submission requires focused local evidence. Work on the next stacked slice may
start while hosted checks run. Merge and phase acceptance still require green
exact-head hosted CI and the applicable terminal evidence. A failed shared
foundation check stops dependent submission until repaired, but a pending check
does not idle independent implementation.
...
the roadmap summary. `Required` means terminal phase evidence, not a command
that must be repeated for every implementation PR. The cadence column controls
when the evidence is gathered.

| Gate                                        | Lab                     | Better Text               | Better Divider            | Better List               | Generator/canary          | Cadence                                                          |
| ------------------------------------------- | ----------------------- | ------------------------- | ------------------------- | ------------------------- | ------------------------- | ---------------------------------------------------------------- |
| React 17 singleton and peer audit           | Required                | Required                  | Required                  | Required                  | Required                  | Dependency change and phase integration                          |
| TypeScript, lint, and focused tests         | Required                | Required                  | Required                  | Required                  | Required                  | Every implementation PR, scoped to touched packages              |
| Full local integration gates                | Required                | Required                  | Required                  | Required                  | Required                  | Frozen phase stack tip only                                      |
| Role, keyboard, focus, Escape, and Axe      | Required                | Required                  | Required                  | Required                  | Required                  | Touched interaction per PR; complete matrix at phase integration |
| Theme, portal, ID, and host-style lifecycle | Required                | Required                  | Required                  | Required                  | Required                  | Touched contract per PR; complete matrix at phase integration    |
| Clean `single` artifact                     | Harness/canary required | Required                  | Required                  | Required                  | Required                  | Applicable phase integration                                     |
| Clean `cdn` artifact                        | Harness/canary required | Required                  | Required                  | Required                  | Required                  | Applicable phase integration                                     |
| Strict `staging-cdn` local proof            | Harness/canary required | Required                  | Required                  | Required                  | Required                  | Applicable phase integration                                     |
| Exact-prefix remote bytes and SHA-256       | Harness/canary required | Required before CDN claim | Required before CDN claim | Required before CDN claim | Required before CDN claim | Release operations only                                          |
| Production-provider response headers        | Harness/canary required | Required before CDN claim | Required before CDN claim | Required before CDN claim | Required before CDN claim | Release operations only                                          |
| Standalone clean install/build              | Not applicable          | Required                  | Required                  | Required                  | Required                  | Applicable application phase integration                         |
| Exact-head hosted CI                        | Required                | Required                  | Required                  | Required                  | Required                  | Submitted PR; complete integration jobs at phase tip             |
| App Catalog deployment                      | Canary support          | Required before release   | Required before release   | Required before release   | Required before release   | Release operations only with explicit authority                  |
| Test-site install or update                 | Canary support          | Required before release   | Required before release   | Required before release   | Required before release   | Release operations only with explicit authority                  |
| Real SharePoint runtime and network trace   | Harness support         | Required                  | Required                  | Required                  | Required                  | Applicable phase exit or release claim                           |
| Prior package/resource retention            | Not applicable          | Required before release   | Required before release   | Required before release   | Required before release   | Release operations only                                          |
| Authorized non-production rollback drill    | Canary support          | Required before release   | Required before release   | Required before release   | Required before release   | Release operations only with explicit authority                  |
...
| D10 | Accepted | Use focused implementation gates, one complete integration gate at each frozen phase tip, and release-only operational proof; start rollback-isolated dependent work when the consumed predecessor interface is frozen. | A0             | 2026-08-08 | [Critical-path cadence](#critical-path-delivery-and-validation-cadence)                                                                                                                                            | A current-path defect escapes because a required focused or phase gate was deferred. |
...
| A future registry source shadows an owned local portal-wrapper binding. | A1             | Deferred hardening | A future profile update could bypass portal routing even though the current digest-pinned profile does not contain this shape. | Reject wrapper shadowing/reassignment before admitting a registry payload that contains it; keep current profile acceptance bound to exact reviewed bytes. | [ADR-0001](adr/0001-react17-shadcn-foundation.md), [#82](https://github.com/pbroom/spfx-kit/issues/82)                                                                                                                                   | Before next registry profile update |
...
4. Add or update focused tests with the implementation. Run the complete phase
   integration suite once at the frozen phase stack tip.
