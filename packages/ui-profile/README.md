...
The repository host remains on `@types/react@17.0.45`. Base UI 1.6.0 declarations
use `React.JSX`, so the strict profile compiler intentionally isolates
`@types/react@17.0.93` with `@types/scheduler@0.16.8`. Both TypeScript matrix runs
list and reject type files outside that profile-local root; this does not alter
the Lab, Workbench, or SPFx host type contract. Provenance records the host and
isolated compiler declarations as separate contracts. The profile ID plus its
