  ...
  it('derives the SCSS-only profile without workspace or Tabs source while retaining exact scoped CSS', async () => {
    ...
    expect(scssOnly.manifest).toMatchObject({
      ...
      css: {
        ...
        scopeSelector: '[data-spfx-ui-scope="skui-d0cb51634265e868"]'
      }
    });
    ...
    expect(runtimeContract?.source).toContain('export const SPFX_UI_SCOPE_VALUE = "skui-d0cb51634265e868" as const;');
