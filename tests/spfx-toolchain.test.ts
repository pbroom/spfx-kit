...
describe('SPFx toolchain model', () => {
  ...
  it('keeps standalone build scripts on the detected toolchain', () => {
    ...
    expect(standaloneScriptsForToolchain('heft', { uiProfile: true }).build).toMatch(/^spfx-ui-profile-verify && heft/u);
    expect(standaloneScriptsForToolchain('gulp', { uiProfile: true }).ship).toMatch(/^spfx-ui-profile-verify && gulp/u);
  });
