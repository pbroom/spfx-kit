...
describe('managed app source roots', () => {
  ...
  it('reconciles app roots discovered, removed, or repointed after startup', async () => {
    ...
    const initial = refreshManagedAppSourceRoots(['/workspace'], [], appsDir);
    expect(initial).toEqual({ allowedRoots: ['/workspace', firstAppRoot], managedRoots: [firstAppRoot] });
    ...
    const refreshed = refreshManagedAppSourceRoots(initial.allowedRoots, initial.managedRoots, appsDir);
    expect(refreshed).toEqual({
      allowedRoots: ['/workspace', firstAppRoot, secondAppRoot],
